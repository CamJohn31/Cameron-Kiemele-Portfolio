...
import { Mail, Phone, Linkedin, MapPin, ChevronDown, Briefcase, GraduationCap, Award, Menu, X } from 'lucide-react';
...
export default function Home() {
  ...
  <div className="absolute inset-0 bg-gradient-to-b from-slate-900 via-transparent to-slate-900"></div>
      ...
      {/* Navigation - Side Popup on Hover */}
      <div 
      ...
      >
        ...
        <div className={`
          ...
          py-8 px-2 bg-emerald-500/10 border-r border-emerald-500/30 backdrop-blur-sm
        `}>
          <div className="text-emerald-400 text-xs font-bold tracking-widest">SECTIONS</div>
        </div>

        {/* Popup Menu */}
        <div className={`
          fixed left-0 top-6 transition-all duration-300 ease-out
          ${isNavHovered ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-full pointer-events-none'}
          bg-slate-900/95 backdrop-blur-sm border-r-2 border-emerald-500/40 min-h-screen py-12 px-6 w-56
        `}>
          <div className="space-y-2">
            {['about', 'background', 'experience', 'education', 'skills'].map((section) => (
              <button
                ...
                className={`
                  w-full text-left capitalize px-4 py-3 rounded-lg transition-all duration-300
                  transform hover:scale-110 origin-left
                  ${activeSection === section
                    ? 'text-emerald-300 bg-emerald-500/20 font-semibold'
                    : 'text-gray-400 hover:text-emerald-400 hover:bg-slate-800/50'
                  }
                `}
              >
                {section}
